...
func Test_packetConnSetNonblock(t *testing.T) {
	...
	// Test 2: should become nonblocking due to timer after now
	...
	// Test 3: should remain nonblocking due to time after now, but not triggered
	if err := p.SetDeadline(time.Now().Add(10 * time.Second)); err != nil {
		t.Fatal(err)
	}
	if want, got := false, s.triggered; want != got {
	...
	// Test 5: socket remain blocking due to zero time, but not trigger
	if err := p.SetDeadline(time.Time{}); err != nil {
		t.Fatal(err)
	}
	if want, got := false, s.triggered; want != got {
